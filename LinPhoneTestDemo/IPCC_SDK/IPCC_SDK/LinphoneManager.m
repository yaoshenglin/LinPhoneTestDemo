...
{
    NSDictionary *dic = @{@(LinphoneRegistrationNone):@"Registration None",
                          @(LinphoneRegistrationProgress):@"Registration Progress",
                          @(LinphoneRegistrationOk):@"Registration successful",
                          @(LinphoneRegistrationCleared):@"Registration Cleared",
                          @(LinphoneRegistrationFailed):@"Registration Failed"};
    NSString *stateMsg = [dic objectForKey:@(state)] ?: @"Unknow";
    return stateMsg;
}
...
- (BOOL)migrateChatDBIfNeeded:(LinphoneCore*)lc {
    ...
    default_proxy = linphone_core_get_default_proxy_config(theLinphoneCore);
	...
	if( shouldMigrate ){
		...
		if( default_proxy ){
            ...
            NSLog(@"%s,%s,%s",identity,value1,value2);
		}
		if( !identity ){
...
- (void)onRegister:(LinphoneCore *)lc cfg:(LinphoneProxyConfig*) cfg state:(LinphoneRegistrationState) state message:(const char*) message {
    ...
    [UCSIPCCSDKLog saveDemoLogInfo:@"登陆状态更新" withDetail:[NSString stringWithFormat:@"state:%@,\nmessage:%@", stateMsg, [NSString stringWithUTF8String:message]]];
		...
		if ((flags == 0) || (flags & networkDownFlags)) {
			...
			switch (lLinphoneMgr.tunnelMode) {
				case tunnel_wwan:
                    ...
                    linphone_tunnel_set_mode(tunnel, lLinphoneMgr.connectivity == wwan);
					break;
				case tunnel_auto:
...
//    libmsilbc_init();
    ...
    linphone_core_set_log_collection_path([[LinphoneLogger cacheDirectory] UTF8String]);
    [LinphoneLogger enableLogs:NO];


    //LinphoneFactory *factory = linphone_factory_get();
    //__unused LinphoneCoreCbs *cbs = linphone_factory_create_core_cbs(factory);
    //theLinphoneCore = linphone_factory_create_core_with_config(factory, cbs, configDb);
    theLinphoneCore = linphone_core_new_with_config (&linphonec_vtable ,configDb ,self /* user_data */);
...
- (void)cancelLocalNotifTimerForCallId:(NSString*)callid {
    ...
    MSList* call = bctbx_list_find_custom(calls, (bctbx_compare_func)comp_call_id, [callid UTF8String]);
    if (call != NULL) {
...
- (void)acceptCall:(LinphoneCall *)call
{
    ...
    BOOL enable = linphone_call_params_video_enabled(params);
    
    [self acceptCall:call enableVideo:enable];
}

- (void)acceptCall:(LinphoneCall *)call enableVideo:(BOOL)enable
{
    ...
    linphone_call_params_enable_video(lcallParams, enable);//视频通话
...
- (void)call:(NSString *)address displayName:(NSString*)displayName transfer:(BOOL)transfer enableVideo:(BOOL)enable
{
	...
	if (linphoneAddress) {
		...
		if(transfer) {
            ...
            linphone_call_transfer(linphone_core_get_current_call(theLinphoneCore), [address cStringUsingEncoding:[NSString defaultCStringEncoding]]);
		} else {
...
- (void)setPushNotificationToken:(NSData *)apushNotificationToken {
    ...
    cfg = linphone_core_get_default_proxy_config(theLinphoneCore);
    if (cfg ) {
